...
const subtopic = topics.find(({id}) =>id === match.params.topicId).resources.find(({id}) => id === match.params.subId)
...
function Topic ({ match }) {
  ...
  console.log(match.params.topicId, "IDDDDD")
  ...
  return (
    <div>
      ...
      {topic.resources.map((sub)=>(
        <li key ={sub.id}>
          {topic.resources.id === match.params.topicId.sub}
          <Link to={`${match.url}/${sub.id}`}>{sub.name}</Link></li>
        ))}
        {console.log(match.params.topicId, "subIDstill undifined")}
      </ul>
      <hr />
        <Route path = {`${match.path}/:subId`}component={Resources}></Route>
    </div>
  )
}
